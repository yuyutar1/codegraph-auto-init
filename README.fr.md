...
2. **Installe un wrapper `git` pour zsh / bash / fish**
...
| `~/.config/codegraph-auto-init/git-wrapper.sh` | Le wrapper pour zsh / bash. Après un `git init` / `git clone` réussi, il détecte le nouveau dépôt et exécute `codegraph init` en arrière-plan |
| `~/.zshrc` / `~/.bashrc` | Une ligne de source est ajoutée dans chacun (marquée par `# codegraph-auto-init`, uniquement si le shell est installé) |
| `~/.config/fish/conf.d/codegraph-auto-init.fish` | La version fish du wrapper, chargée automatiquement par fish (installée uniquement si fish est présent) |
...
| zsh / bash / fish | auto-init lors de `git init` / `git clone` | le wrapper est installé pour chaque shell présent sur la machine ; le ignore, le CLI et le scan fonctionnent avec n'importe quel shell |
